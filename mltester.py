...
def read_store(path: Path) -> pd.DataFrame:
    ...
    # --- FIXED DATE PARSING SECTION ---
    try:
        # Automatically infer multiple formats (handles DD-MM-YYYY, YYYY-MM-DD, etc.)
        df["date"] = pd.to_datetime(df["date"], format="mixed", dayfirst=True, errors="raise")
    except Exception as e:
        print(f"⚠️ Date parsing error: {e}")
        print("Retrying with flexible parsing...")
        # Fallback parser for messy dates
        df["date"] = pd.to_datetime(df["date"], infer_datetime_format=True, dayfirst=True, errors="coerce")
        if df["date"].isna().any():
            bad_rows = df[df["date"].isna()].head()
            raise ValueError(f"Unrecognized date format found in {path}. Example(s):\n{bad_rows}")
